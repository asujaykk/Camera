...
#        photo = '/photo.jpg'
    ...
    #     pic = '/shot.jpg'
    #     url_response= (self.httpRequest(self.url +pic))
    ...
    def setZoom(self, zoom_value,zoom_min = 0,zoom_max=100):
        ...
        if zoom_value >= zoom_min and zoom_value <= zoom_max:
            self.httpRequest( self.url + zoom_cam + str(zoom_value))
    ...
    def setQuality(self, quality_val,qual_min = 1,qual_max =100):
        ...
        if quality_val >= qual_min and quality_val <= qual_max:
            self.httpRequest( self.url + qual_cam + str(quality_val))
    ...
    def A_setExposure(self, exp_val,expo_min = 10783,expo_max = 31617300000):
        ...
        self.httpRequest( self.url + expo_mode ) #setting expo mode to manual first
        
        if exp_val >= expo_min and exp_val <= expo_max:
            self.httpRequest( self.url + expo_cam + str(exp_val))
    ...
    def flashOn(self):
        ...
        self.httpRequest( self.url + torch_on)
    
    def flashOff(self):
        torch_off = '/disabletorch'
        self.httpRequest( self.url + torch_off)
    
    def focus(self):
        focus = '/focus'
        self.httpRequest( self.url + focus)
    ...
    def A_setFocus(self,focus_val,focus_min = 0,focus_max=13):
        ...
        self.httpRequest( self.url + focus_mode ) #setting focus mode to manula first
        
        if focus_val >= focus_min and focus_val <= focus_max:
            self.httpRequest( self.url + focus_cam + str(focus_val))
    ...
    def A_setFocusMode(self,focus_mode = auto):
        ...
        focus_mode==IPCam.macro or focus_mode==IPCam.smooth or focus_mode==IPCam.aggressive: 
           ...
           self.httpRequest( self.url + focus_mode ) #setting focus mode
           return 0
        else:
    ...
    def noFocus(self):
        nofocus = '/nofocus'
        self.httpRequest( self.url + nofocus)
        return 0
    ...
    def A_setSensorControl(self,sensor_mode=auto):
        ...
        if sensor_mode==IPCam.auto:
            self.httpRequest( self.url + auto_cam)
        elif sensor_mode==IPCam.manual:
            self.httpRequest( self.url + manual_cam)
    
    def A_setIso(self,iso_val,iso_min = 100,iso_max=5699):
        iso_inc = 1
        iso_mode= '/settings/manual_sensor?set=on'
        iso_cam = '/settings/iso?set='
        self.httpRequest( self.url + iso_mode ) #setting iso mode to manula first
        
        if iso_val >= iso_min and iso_val <= iso_max:
            self.httpRequest( self.url + iso_cam + str(iso_val))
    ...
    def A_setShutter(self,shutter_val,shutter_min = 33333333,shutter_max=31617425424):
        ...
        self.httpRequest( self.url + shutter_mode ) #setting shutter mode to manula first
        
        if shutter_val >= shutter_min and shutter_val <= shutter_max:
            self.httpRequest( self.url + shutter_cam + str(shutter_val))
    ...
    def switchToFrontCam(self):
        ...
        self.httpRequest( self.url + fron_cam)
        return 0
        
    def switchToRearCam(self):
        main_cam = '/settings/ffc?set=off'
        self.httpRequest( self.url + main_cam)
        return 0
     
    def sensorData(self):
        sensor = '/sensors.json'
        return(self.httpRequest( self.url + sensor))
